...
const designationApi = hrmApi.injectEndpoints({
  endpoints: (build) => ({
    getDesignation: build.query<{ data: DesignationColumn[] }, void>({
      query: () => "designations",
    ...
    removeDesignation: build.mutation<DeleteResponse, number>({
      ...
      invalidatesTags: ["designations"],
		...
		updateDesignation: build.mutation<{ data: DesignationColumn }, { designationId: number, updatedDesignation: DesignationFromValues }>({
