...
import { useCreatePurchaseRefundMutation } from "@/store/services/billing/api/purchase-refund";

interface AddPaymentReceivedProps {
  modalClose: () => void;
}

export function AddPurchaseRefundFrom({ modalClose }: AddPaymentReceivedProps) {
  const [selectedProducts, setSelectedProducts] = useState<PurchaseRow[]>([]);
  const [contactId, setContactId] = useState<number | null>(null);
  const [receivedAmount, setReceivedAmount] = useState<number>(0);
  const [uploadedFiles, setUploadedFiles] = useState<File[]>([]);

  const [createPurchaseRefund, { isLoading }] =
  ...
  async function onSubmit(data: BillingPaymentFormType) {
    if (selectedProducts.length === 0) {
      return toast.error("Please select at least one supplier");
    }
    ...
    try {
      await createPurchaseRefund(formData).unwrap();
      toast.success("Purchase Refund created successfully");
      modalClose();
    } catch (error) {
      console.log(error);
