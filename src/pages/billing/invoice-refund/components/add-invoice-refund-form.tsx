...
import { useCreateInvoiceRefundMutation } from "@/store/services/billing/api/invoice-refund";
...
export function AddPurchaseRefundFrom({ modalClose }: AddPaymentReceivedProps) {
  ...
  const [createInvoiceRefund, { isLoading }] = useCreateInvoiceRefundMutation();
  ...
  async function onSubmit(data: BillingPaymentFormType) {
    ...
    try {
      await createInvoiceRefund(formData).unwrap();
      toast.success("Invoice Refund created successfully");
      modalClose();
    } catch (error) {
      console.log(error);
