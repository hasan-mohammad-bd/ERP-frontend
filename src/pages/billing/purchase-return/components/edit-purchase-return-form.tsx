...
export default function EditPurchaseReturn() {
  ...
  const { data: purchaseOrderData, isLoading: isQuotationLoading } =
    useGetPurchaseOrderByIdQuery(purchaseOrderId, { skip: !purchaseOrderId });
  ...
  useEffect(() => {
    if (purchaseOrderData) {
      form.reset({
        ...
        discount: Number(purchaseOrderData.data.discount),
        total: Number(purchaseOrderData.data.total),
        shipping_charges: Number(purchaseOrderData.data.shipping_charges),
        reference: purchaseOrderData.data.reference ?? undefined,
      });

      setSelectedProducts(
        purchaseOrderData.data.details.map((detail) => ({
