...
import { useCreateEmployeeClassMutation, useUpdateEmployeeClassMutation } from "@/store/services/hrm/api/employee-class";

interface AddSectionFormProps { modalClose: () => void; data?: SectionColumn; }

export function AddEmployeeClassForm({ modalClose, data: previousData,  }: AddSectionFormProps ) {
	const [createEmployeeClass, { isLoading }] = useCreateEmployeeClassMutation();
	const [updateEmployeeClass, { isLoading: updateLoading }] = useUpdateEmployeeClassMutation();
	...
	async function onSubmit(data: SectionFromValues) {
		try {
			if(previousData){
				await updateEmployeeClass({ employeeClassId: previousData.id, updatedEmployeeClass: data });
							...
							render={({ field }) => (
								<FormItem>
									...
									<FormControl>
										<Input
											type="number"
											placeholder="Enter employee class sorting index"
											{...field}
										/>
									</FormControl>
									<FormMessage />
